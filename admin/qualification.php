    ...
    <section class="user-dashboard">
      <div class="dashboard-outer">
        ...
        <div class="row">
              ...
              <div class="tabs-box">
                <div class="widget-title">
                  ...
                  <div class="chosen-outer">
                    <!--Tabs Box-->
                    <button class="theme-btn btn-style-one" type="button" id="openqualificationModal"><i class="fa fa-tasks"></i></button>
                  </div>
                </div>
          ...
          <div id="qualification-modal-crte" style="display: none;">
            <div class="model">
              <!-- Login modal -->
                ...
                <div class="login-form default-form">
                  <div class="form-inner">
                    ...
                    <form id="qualificationForm" method="post">
                      ...
                      <div class="form-group">
                          <button class="theme-btn btn-style-one" type="button" id="submitQualification">Add</button>
          ...
          <div id="qualification-modal-update" style="display: none;">
              ...
              <div style="max-width: 500px;padding: 30px 40px 20px; overflow: visible; background: #fff; border-radius: 8px; box-shadow: none;">
                ...
                <div class="login-form default-form">
                  <div class="form-inner">
                    ...
                    <input type="hidden" name="equalification_id" id="equalification_id" placeholder="Qualification" required>
                      <div class="form-group">
                          <div class="row" style="margin-bottom: 20px;">
                              <div class="form-group col-lg-12 col-md-12">
                                  <label>Qualification</label>
                                  <input type="text" name="equalification" id="equalification" placeholder="Qualification" required>
                              </div>
                      ...
                      <div class="form-group">
                          <button class="theme-btn btn-style-two" type="button" id="updateQualification">Update</button>
  ...
  <?php include'../web-layouts/scripts.php';?>
          ...
          <script>
            $(document).ready(function () {
                $("#openqualificationModal").click(function () {
                    $("#qualification-modal-crte").modal("show");
                });
                  // When the "Add" button is clicked
                $("#submitQualification").click(function () {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "../function/admin/a_qualification_crte.php", // The PHP script on the current page to handle the form data
                        data: formData,
...
<script>
    $(document).ready(function () {
        ...
        function fetchJobAppData(page) {
            $.ajax({
                url: '../function/admin/a_qualification_fetch.php',
                method: 'GET',
                dataType: 'json',
                ...
                success: function (data) {
                    if (data && data.length > 0) {
                        ...
                        data.forEach(function (data) {
                            var row = '<tr>' +
                                '<td>' +
                                '<span class="info">' + data.qualification + '</span>' +
                                '</td>' +
                                '<td>' +
                                '<div class="option-box">' +
                                '<ul class="option-list">' +
                                '<li><button data-user-id="' + data.qualification_id + '" data-text="Edit"><span class="la la-pencil"></span></button></li>' +
                                '<li><button data-user-id="' + data.qualification_id + '" data-text="Delete"><span class="la la-trash"></span></button></li>' +
                                '</ul>' +
                                '</div>' +
                                '</td>' +
                                '</tr>';
                        ...
                        $('#qualification-list').html(tableContent);
                        ...
                        $('#qualification-list').html('<tr><td colspan="2">No qualification found.</td></tr>');
                    }
                },
        ...
        fetchJobAppData(1);
            ...
            $('#qualification-list').on('click', 'button[data-text="Edit"]', function () {
                // Get the category data from the clicked row
                var qualification_id = $(this).data('user-id');
                var qualification = $(this).closest('tr').find('.info').text();

                // Populate the modal with the data
                $('#equalification_id').val(qualification_id);
                $('#equalification').val(qualification);

                // Show the modal
                $("#qualification-modal-update").modal("show");
            });

            $('#qualification-list').on('click', 'button[data-text="Delete"]', function () {
              var qualification_id = $(this).data('user-id');
              ...
              Swal.fire({
                  ...
                  if (result.isConfirmed) {
                      ...
                      $.ajax({
                          url: '../function/delete_qualification.php', // Update the URL
                          method: 'POST',
                          data: { equalification_id: qualification_id },
                          dataType: 'json',
                          success: function (response) {
                              if (response.success) {
      ...
      $(document).ready(function () {
        $('#updateQualification').click(function () {
            var qualification_id = $('#equalification_id').val();
            var newQualificationName = $('#equalification').val();

            

            // Make an AJAX request to update the category data
            $.ajax({
                url: '../function/update_qualification.php',
                method: 'POST',
                data: {
                  equalification_id: qualification_id,
                  equalification: newQualificationName
                },
                dataType: 'json',
                success: function (response) {
                    if (response.message) {
                  ...
                  error: function () {
                      Swal.fire({
                          icon: 'error',
                          title: 'Error',
                          text: 'Failed to update qualification',
                          confirmButtonText: 'OK'
                      });
                  }
                });
