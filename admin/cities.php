    ...
    <section class="user-dashboard">
      <div class="dashboard-outer">
        ...
        <div class="row">
              ...
              <div class="tabs-box">
                <div class="widget-title">
                  ...
                  <div class="chosen-outer">
                    <!--Tabs Box-->
                    <button class="theme-btn btn-style-one" type="button" id="opencitiesModal"><i class="fa fa-tasks"></i></button>
                  </div>
                </div>
          ...
          <div id="cities-modal-crte" style="display: none;">
            <div class="model">
              <!-- Login modal -->
                ...
                <div class="login-form default-form">
                  <div class="form-inner">
                    ...
                    <form id="citiesForm" method="post">
                      <div class="form-group">
                          <div class="row" style="margin-bottom: 20px;">
                              <div class="form-group col-lg-12 col-md-12">
                                  <label>Cities</label>
                                  <input type="text" name="cities" placeholder="Cities" required>
                              </div>
                      ...
                      <div class="form-group">
                          <button class="theme-btn btn-style-one" type="button" id="submitCities">Add</button>
          ...
          <div id="cities-modal-update" style="display: none;">
              ...
              <div style="max-width: 500px;padding: 30px 40px 20px; overflow: visible; background: #fff; border-radius: 8px; box-shadow: none;">
                ...
                <div class="login-form default-form">
                  <div class="form-inner">
                    ...
                    <input type="hidden" name="ecities_id" id="ecities_id" placeholder="City" required>
                      <div class="form-group">
                          <div class="row" style="margin-bottom: 20px;">
                              <div class="form-group col-lg-12 col-md-12">
                                  <label>City</label>
                                  <input type="text" name="ecities" id="ecities" placeholder="City" required>
                              </div>
                      ...
                      <div class="form-group">
                          <button class="theme-btn btn-style-two" type="button" id="updateCities">Update</button>
  ...
  <?php include'../web-layouts/scripts.php';?>
          ...
          <script>
            $(document).ready(function () {
                $("#opencitiesModal").click(function () {
                    $("#cities-modal-crte").modal("show");
                });
                  // When the "Add" button is clicked
                $("#submitCities").click(function () {
                    // Serialize the form data
                    var formData = $("#citiesForm").serialize();

                    // Send an AJAX POST request to the current page
                    $.ajax({
                        type: "POST",
                        url: "../function/admin/a_cities_crte.php", // The PHP script on the current page to handle the form data
                        data: formData,
...
<script>
      $(document).ready(function () {
        ...
        function fetchJobAppData(page) {
            $.ajax({
                url: '../function/admin/a_cities_fetch.php',
                method: 'GET',
                dataType: 'json',
                ...
                success: function (data) {
                    if (data && data.length > 0) {
                        ...
                        data.forEach(function (data) {
                            var row = '<tr>' +
                                '<td>' +
                                '<span class="info">' + data.cities + '</span>' +
                                '</td>' +
                                '<td>' +
                                '<div class="option-box">' +
                                '<ul class="option-list">' +
                                '<li><button data-user-id="' + data.cities_id + '" data-text="Edit"><span class="la la-pencil"></span></button></li>' +
                                '<li><button data-user-id="' + data.cities_id + '" data-text="Delete"><span class="la la-trash"></span></button></li>' +
                                '</ul>' +
                                '</div>' +
                                '</td>' +
                                '</tr>';
                        ...
                        $('#cities-list').html('<tr><td colspan="2">No cities found.</td></tr>');
                    }
                },
        ...
        fetchJobAppData(1);
            ...
            $('#cities-list').on('click', 'button[data-text="Edit"]', function () {
                // Get the category data from the clicked row
                var cities_id = $(this).data('user-id');
                var cities = $(this).closest('tr').find('.info').text();

                // Populate the modal with the data
                $('#ecities_id').val(cities_id);
                $('#ecities').val(cities);

                // Show the modal
                $("#cities-modal-update").modal("show");
            });

            $('#cities-list').on('click', 'button[data-text="Delete"]', function () {
              var cities_id = $(this).data('user-id');
              ...
              Swal.fire({
                  ...
                  if (result.isConfirmed) {
                      ...
                      $.ajax({
                          url: '../function/delete_city.php', // Update the URL
                          method: 'POST',
                          data: { ecities_id: cities_id },
                          dataType: 'json',
                          success: function (response) {
                              if (response.success) {
      ...
      $(document).ready(function () {
        $('#updateCities').click(function () {
            var cities_id = $('#ecities_id').val();
            var newCityName = $('#ecities').val();

            

            // Make an AJAX request to update the category data
            $.ajax({
                url: '../function/update_city.php',
                method: 'POST',
                data: {
                  ecities_id: cities_id,
                  ecities: newCityName
                },
                dataType: 'json',
                success: function (response) {
