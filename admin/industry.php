    ...
    <section class="user-dashboard">
      <div class="dashboard-outer">
        ...
        <div class="row">
              ...
              <div class="tabs-box">
                <div class="widget-title">
                  ...
                  <div class="chosen-outer">
                    <!--Tabs Box-->
                    <button class="theme-btn btn-style-one" type="button" id="openindustryModal"><i class="fa fa-tasks"></i></button>
                  </div>
                </div>
          ...
          <div id="industry-modal-crte" style="display: none;">
            <div class="model">
              <!-- Login modal -->
                ...
                <div class="login-form default-form">
                  <div class="form-inner">
                    ...
                    <form id="industryForm" method="post">
                      ...
                      <div class="form-group">
                          <button class="theme-btn btn-style-one" type="button" id="submitIndustry">Add</button>
          ...
          <div id="industry-modal-update" style="display: none;">
              ...
              <div style="max-width: 500px;padding: 30px 40px 20px; overflow: visible; background: #fff; border-radius: 8px; box-shadow: none;">
                ...
                <div class="login-form default-form">
                  <div class="form-inner">
                    ...
                    <input type="hidden" name="eindustry_id" id="eindustry_id" placeholder="Industry" required>
                      <div class="form-group">
                          <div class="row" style="margin-bottom: 20px;">
                              <div class="form-group col-lg-12 col-md-12">
                                  <label>Industry</label>
                                  <input type="text" name="eindustry" id="eindustry" placeholder="Industry" required>
                              </div>
                      ...
                      <div class="form-group">
                          <button class="theme-btn btn-style-two" type="button" id="updateIndustry">Update</button>
  ...
  <?php include'../web-layouts/scripts.php';?>
          ...
          <script>
            $(document).ready(function () {
                $("#openindustryModal").click(function () {
                    $("#industry-modal-crte").modal("show");
                });
                ...
                $("#submitIndustry").click(function () {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "../function/admin/a_industry_crte.php", // The PHP script on the current page to handle the form data
                        data: formData,
...
<script>
    $(document).ready(function () {
        ...
        function fetchJobAppData(page) {
            $.ajax({
                url: '../function/admin/a_industry_fetch.php',
                method: 'GET',
                dataType: 'json',
                ...
                success: function (data) {
                    if (data && data.length > 0) {
                        ...
                        data.forEach(function (data) {
                            var row = '<tr>' +
                                '<td>' +
                                '<span class="info">' + data.industry + '</span>' +
                                '</td>' +
                                '<td>' +
                                '<div class="option-box">' +
                                '<ul class="option-list">' +
                                '<li><button data-user-id="' + data.industry_id + '" data-text="Edit"><span class="la la-pencil"></span></button></li>' +
                                '<li><button data-user-id="' + data.industry_id + '" data-text="Delete"><span class="la la-trash"></span></button></li>' +
                                '</ul>' +
                                '</div>' +
                                '</td>' +
                                '</tr>';
                        ...
                        $('#industry-list').html(tableContent);
                        ...
                        $('#industry-list').html('<tr><td colspan="2">No industry found.</td></tr>');
                    }
                },
        ...
        fetchJobAppData(1);
            ...
            $('#industry-list').on('click', 'button[data-text="Edit"]', function () {
                // Get the category data from the clicked row
                var industry_id = $(this).data('user-id');
                var industry = $(this).closest('tr').find('.info').text();

                // Populate the modal with the data
                $('#eindustry_id').val(industry_id);
                $('#eindustry').val(industry);

                // Show the modal
                $("#industry-modal-update").modal("show");
            });

            $('#industry-list').on('click', 'button[data-text="Delete"]', function () {
              var industry_id = $(this).data('user-id');
              ...
              Swal.fire({
                  ...
                  if (result.isConfirmed) {
                      ...
                      $.ajax({
                          url: '../function/delete_industry.php', // Update the URL
                          method: 'POST',
                          data: { eindustry_id: industry_id },
                          dataType: 'json',
                          success: function (response) {
                              if (response.success) {
      ...
      $(document).ready(function () {
        $('#updateIndustry').click(function () {
            var industry_id = $('#eindustry_id').val();
            var newIndustryName = $('#eindustry').val();

            

            // Make an AJAX request to update the category data
            $.ajax({
                url: '../function/update_industry.php',
                method: 'POST',
                data: {
                  eindustry_id: industry_id,
                  eindustry: newIndustryName
                },
                dataType: 'json',
                success: function (response) {
                    if (response.message) {
