    ...
    <section class="user-dashboard">
      <div class="dashboard-outer">
        ...
        <div class="row">
          ...
          <div id="categories-modal-crte" style="display: none;">
            <div class="model">
              <!-- Login modal -->
                ...
                <div class="login-form default-form">
                  <div class="form-inner">
                    ...
                    <form id="categoriesForm" method="post">
                      ...
                      <div class="form-group">
                          <button class="theme-btn btn-style-one" type="button" id="submitCategories">Add</button>
          ...
          <div id="categories-modal-update" style="display: none;">
              ...
              <div style="max-width: 500px;padding: 30px 40px 20px; overflow: visible; background: #fff; border-radius: 8px; box-shadow: none;">
                ...
                <div class="login-form default-form">
                  <div class="form-inner">
                    ...
                    <input type="hidden" name="ecategories_id" id="ecategories_id" placeholder="Categories" required>
                      <div class="form-group">
                          <div class="row" style="margin-bottom: 20px;">
                              <div class="form-group col-lg-12 col-md-12">
                                  <label>Categories</label>
                                  <input type="text" name="ecategories" id="ecategories" placeholder="Categories" required>
                              </div>
                      ...
                      <div class="form-group">
                          <button class="theme-btn btn-style-two" type="button" id="updateCategories">Update</button>
          ...
          <script>
            $(document).ready(function () {
                $("#openskillModal").click(function () {
                    $("#categories-modal-crte").modal("show");
                });
                  // When the "Add" button is clicked
                $("#submitCategories").click(function () {
                    // Serialize the form data
                    var formData = $("#categoriesForm").serialize();

                    // Send an AJAX POST request to the current page
                    $.ajax({
                        type: "POST",
                        url: "../function/admin/a_categories_crte.php", // The PHP script on the current page to handle the form data
                        data: formData,
        ...
        <script>
          $(document).ready(function () {
              ...
              function fetchJobAppData(page) {
                  $.ajax({
                      url: '../function/admin/a_categories_fetch.php',
                      method: 'GET',
                      dataType: 'json',
                          ...
                          if (data && data.length > 0) {
                              ...
                              data.forEach(function (data) {
                                  var row = '<tr>' +
                                      ...
                                      '<li><button data-user-id="' + data.categories_id + '" data-text="Edit"><span class="la la-pencil"></span></button></li>' +
                                      '<li><button data-user-id="' + data.categories_id + '" data-text="Delete"><span class="la la-trash"></span></button></li>' +
                                      '</ul>' +
                                      '</div>' +
                                      '</td>' +
                                      '</tr>';
                              ...
                              $('#skills-list').html('<tr><td colspan="2">No job applications found.</td></tr>');
                          }
                      },
              ...
              $('#skills-list').on('click', 'button[data-text="Edit"]', function () {
                ...
                var categories_id = $(this).data('user-id');
                var category_name = $(this).closest('tr').find('.info').text();

                // Populate the modal with the data
                $('#ecategories_id').val(categories_id);
                $('#ecategories').val(category_name);

                // Show the modal
                $("#categories-modal-update").modal("show");
            });
            ...
            $('#skills-list').on('click', 'button[data-text="Delete"]', function () {
              ...
              Swal.fire({
                  ...
                  if (result.isConfirmed) {
                      ...
                      $.ajax({
                          url: '../function/delete_category.php', // Update the URL
                          method: 'POST',
                          data: { ecategories_id: categories_id },
                          dataType: 'json',
                          success: function (response) {
                              if (response.success) {
          ...
          // Add an event handler for the "Update" button
          $('#updateCategories').click(function () {
            var categories_id = $('#ecategories_id').val();
            var newCategoryName = $('#ecategories').val();


            // Make an AJAX request to update the category data
            $.ajax({
                url: '../function/update_category.php',
                method: 'POST',
                data: {
                    ecategories_id: categories_id,
                    ecategories: newCategoryName
                },
                dataType: 'json',
                success: function (response) {
                    if (response.message) {
